...
STATICFILES_VENDORE_DIR = getattr(settings, 'STATICFILES_VENDOR_DIR')
...
class Command(BaseCommand):
    ...
    def handle(self, *args:Any, **options:Any):
        self.stdout.write("downloading the vendor staticfiles")
        completed_urls = []

        for name, url in VENDOR_STATICFILES.items():
            out_path = STATICFILES_VENDORE_DIR / name
        ...
        if set(completed_urls) == set(VENDOR_STATICFILES.values()):
            self.stdout.write(
                    self.style.SUCCESS('Succesfully updated the vendor static files.')
                )
        else:
            self.stdout.write(
